...
int SysTime::sysTime_get(GW_TIME * timeStruct) {
	time_t currentTime;
	struct tm *timeptr;
	//char line[MAX_LINE];
...
/////////////////////////////////////////////////////
int SysTime::sysTime_toString(char* currDate, char* currTime) {
	time_t currentTime;
	struct tm *timeptr;
	//char line[MAX_LINE];
...
/////////////////////////////////////////////////////
// currDate format: 20100324 (for 24.03.2010) // 9 char includ. nullterm.
/////////////////////////////////////////////////////
int SysTime::sysDate_toStringNotSeparator(char* currDate) {
	time_t currentTime;
	struct tm *timeptr;
	//char line[MAX_LINE];
	...
	snprintf(currDate, 25, "%.4d%.2d%.2d", timeptr->tm_year + 1900, timeptr->tm_mon + 1, timeptr->tm_mday);

	return timeptr->tm_hour;
}


int SysTime::sysTime_toString2(char* currDateAndTime) {
	time_t currentTime;
	struct tm *timeptr;
	//char line[MAX_LINE];
	...
	currentTime = time((time_t *) NULL);
	timeptr = localtime(&currentTime);
	sprintf(currDateAndTime, "%.2d_%.2d_%.4d %.2d_%.2d_%.2d", timeptr->tm_mday, timeptr->tm_mon
			+ 1, timeptr->tm_year + 1900, timeptr->tm_hour, timeptr->tm_min,
			timeptr->tm_sec);
...
int SysTime::sysTime_toString3(char* currDate, char* currTime) {
	time_t currentTime;
	struct tm *timeptr;
	//char line[MAX_LINE];
...
int SysTime::minutSecondToString(char* currMinAndSecond) {
	time_t currentTime;
	struct tm *timeptr;
	//char line[MAX_LINE];
	...
	currentTime = time((time_t *) NULL);
	timeptr = localtime(&currentTime);
	sprintf(currMinAndSecond, "%.2d:%.2d", timeptr->tm_min,
			timeptr->tm_sec);
