...
function SliderHorizontalUnlinear(slidIndex, slidName, slUnit, slMin, slMax, slStart,
		parentContainer, textArray, textPosArray, bDiscret) {
	...
	for (i = 0; i < textPosArray.length; i++) {
		...
		this.txtDiv.style.left = textPosArray[i]-28 + "px";
		
		this.slidCenter.appendChild(this.txtDiv);
		
		var lineDiv = this.document.createElement("DIV");
		lineDiv.className = "horisSlidLine";
		lineDiv.style.left = textPosArray[i] + "px";
		this.slidCenter.appendChild(lineDiv);
	}
...
SliderHorizontalUnlinear.prototype.changeUnderText = function(newText) {
	this.nameDiv.innerHTML = newText;
};

SliderHorizontalUnlinear.prototype.showSlider = function(show) {
	if (show)
...
SliderHorizontalUnlinear.prototype.convertPosToValue = function() {
	return this.slMin
			+ parseInt(this.currentPos * (this.slMax - this.slMin) / 300);
};

SliderHorizontalUnlinear.prototype.convertValToPosition = function(newValue) {
...
SliderHorizontalUnlinear.prototype.setHandlButtPixPos = function(newX, anim) {
	if(newX >= 300)
		newX = 300;
	else if (newX < 0) {
		newX = 0;
...
SliderHorizontalUnlinear.prototype.setHandlButtValue = function(newValue, anim) {
